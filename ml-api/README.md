...
tidal_data internal (harmonik/stasiun)   ─┘                     (proxy persentil terkalibrasi + ground truth
...
Proyeksi pasut harmonik/stasiun 30 hari  ─┘
```
...
- **Label** (prioritas menaik): (1) proxy persentil per stasiun — pasangan
...
- **Datum pasut**: `tidal_data` = `sea_level_height_msl` Open-Meteo Marine
  ...
  juga meter di atas MSL — tanpa offset karangan.
- **Pasut per stasiun**: model harmonik (M2/S2/K1/O1) di-fit per stasiun —
  fase Samudra Hindia (Pesisir Barat) ≠ Laut Jawa (Tulang Bawang/Mesuji)
  ≠ Teluk Lampung.
