...
- [x] **P2** Backup restore drill: latihan restore sekali sebelum go-live (prosedur sudah ditulis di `docs/backup-database.md`). *(2026-07-20, `(keputusan)` user: restore ke DB test lokal, bukan Supabase production. Dump nyata (24 MB, format custom pg_dump, snapshot Supabase 2026-07-16) ditemukan tersisa dari sesi sebelumnya, dipakai langsung — hemat satu putaran unduh manual, tetap menguji prosedur yang sama persis. Perintah dari dokumentasi (`pg_restore --no-owner --no-privileges --clean --if-exists`) jalan tanpa modifikasi & tanpa error ke `siperah_rob_test` (PostgreSQL 18.3 lokal, client 18.3 — dump sumber Postgres 17, restore lintas versi minor mulus). Verifikasi bukan cuma row count: `regions=2648, predictions=10151, users=8, reports=3` (cocok dengan angka production yang teramati sesi ini), geometri PostGIS lolos `ST_IsValid`+tipe benar (`ST_MultiPolygon`/`ST_Polygon`), 61 index public schema ikut utuh. `tidal_data=0` di dump ini BUKAN kegagalan restore — dump bertanggal 16 Juli, backfill 138rb baris tidal baru terjadi 18 Juli (sebelum dump diambil). Setelah drill, DB test dikembalikan ke state seed bersih (`migrate:fresh --seed`) agar suite PHPUnit/E2E tidak tercemar data production; suite 118 test tetap hijau.)*
