...
- [x] **P1** Audit cakupan wilayah pesisir: cakupan resmi = **8 kabupaten/kota stasiun ML** + temuan 1 desa Lampung Tengah yang polygon-nya menyentuh pantai (Rantau Jaya Ilir, Kec. Putra Rumbia). Final di production 2026-07-17: **315 wilayah pantauan** — 302 hasil klasifikasi PostGIS polygon asli (`ST_DWithin` 1000 m ke garis pantai BIG) + **13 desa Kec. Rawa Jitu Utara (Mesuji) ditandai MANUAL** (keputusan: desa rawa pasang surut di muara, rob merambat lewat sungai, polygon 6,8+ km dari garis pantai — daftar manual dikodifikasi di `ClassifyCoastalRegions::MANUAL_COASTAL_DISTRICTS`). Aproksimasi bbox lama (321) menggelembung 19 wilayah. Prediksi ML harian otomatis mengikuti 315 wilayah mulai run besok.
  - *Sisa kecil (tidak memblokir)*: teks PRD "7 dari 15" perlu dikoreksi jadi cakupan aktual; verifikasi silang 315 vs daftar resmi 283 kelurahan BPS bila daftar itu didapat.
