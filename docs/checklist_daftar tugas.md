...
- [ ] **P1** Queue worker (dibutuhkan Tahap 4). *(2026-07-19: sisi kode SIAP & TERBUKTI — `QueueWorkerTest` membuktikan notifikasi ShouldQueue masuk tabel `jobs` saat `QUEUE_CONNECTION=database` lalu diproses tuntas oleh `queue:work --stop-when-empty` (perintah persis cron Hostinger yang sudah ter-gate `config queue.default === database`) sampai mendarat di inbox tanpa failed job. **Tinggal eksekusi user di server** (butuh tethering): 1) edit `.env` → `QUEUE_CONNECTION=database`; 2) `php artisan config:cache`; 3) uji: submit laporan lalu `SELECT count(*) FROM jobs;` — baris muncul & terkuras ≤1 menit oleh cron; 4) pantau `failed_jobs` beberapa hari pertama.)*
...
- [x] **P2** CI backend: composer install + PHP lint + PHPUnit. CI frontend: npm ci + tsc + build. *(2026-07-19: dua workflow GitHub Actions baru, trigger push/PR ke main dengan path filter (hanya jalan saat backend/frontend berubah) + `workflow_dispatch` manual. **Backend** (`backend-ci.yml`): service container `postgis/postgis:17-3.4-alpine` (meniru `siperah_rob_test` lokal) → `composer install` → lint sintaks (`php -l` semua file, tanpa dependensi linter baru) → `migrate:fresh --seed --force` → `vendor/bin/phpunit`. **Frontend** (`frontend-ci.yml`): `npm ci` → `tsc -b --noEmit` → `npm run build` (termasuk guard ukuran chunk `vite.config.ts`). Ketiga langkah non-service (lint PHP, tsc, build) diverifikasi lulus lokal sebelum commit; PHPUnit di CI belum bisa diverifikasi dari sini (butuh runner GitHub sungguhan) — cek run pertama setelah push.)*
