...
| **Versi** | 0.1 (Draft) |
| **Tanggal** | 8 Juli 2026 |
| **Disusun berdasarkan** | 14 mockup UI/UX (`SIG_Penelitian_2026.zip`) â€” belum ada spesifikasi teknis tertulis sebelumnya |
| **Status** | Draft â€” kebutuhan diturunkan dari desain, perlu divalidasi ulang oleh tim/pembimbing |
...
- Wilayah pesisir Lampung â€” 7 dari 15 kabupaten/kota, mencakup 283 kelurahan pesisir â€” rawan banjir rob, terutama saat kondisi astronomis ekstrem (perigee bertepatan bulan baru/purnama).
...
1. Menyediakan prediksi risiko banjir rob berbasis ML dengan akurasi terukur (baseline mockup: 87%, precision 0.89, recall 0.85) untuk cakrawala 0â€“7 hari.
...
- "Mode Awam" â€” tampilan simplifikasi berbasis lokasi untuk pengguna non-teknis
...
- Pelatihan ulang (retraining) model ML â€” sistem mengonsumsi hasil prediksi, bukan melatih model
- Fitur komersial/pembayaran
- Aplikasi mobile native â€” mockup mengindikasikan web responsive, bukan aplikasi native
- Cakupan di luar Provinsi Lampung

## 6. Kebutuhan Fungsional

### 6.1 Portal Publik â€” Peta & Informasi Risiko (`FR-PUB`)
...
- **FR-PUB-4**: Menampilkan panel detail per wilayah saat diklik â€” nama wilayah, kelas bahaya, probabilitas (%), estimasi populasi berisiko.
...
### 6.2 Mode Awam â€” Status Bahaya Personal (`FR-AWAM`)
...
- **FR-GT-1**: Alur pelaporan 3 langkah â€” (1) pilih lokasi via peta, (2) isi tingkat keparahan & deskripsi, (3) unggah foto & kirim.
- **FR-GT-2**: Klasifikasi keparahan berbasis tinggi genangan: Ringan (<10 cm), Sedang (10â€“30 cm), Parah (30â€“80 cm), Sangat Parah (>80 cm).
- **FR-GT-3**: Sistem mencatat koordinat lokasi, waktu kejadian, dan estimasi tinggi air.
- **FR-GT-4**: Laporan berstatus "menunggu" hingga diverifikasi BPBD, dengan target SLA verifikasi 1Ã—24 jam.

### 6.4 Dashboard BPBD â€” Operator (`FR-OPS`)
...
### 6.5 Dashboard BPBD â€” Provinsi (`FR-PROV`)
...
- **FR-ADM-3**: Sistem mencatat audit log untuk aksi penting: login, logout, validasi laporan, penolakan laporan, ekspor data, perubahan data pengguna, dan update model â€” masing-masing dengan aktor, target, waktu, dan outcome (success/fail/denied/partial).
...
- **FR-NOTIF-1**: Pengguna dapat memilih kanal notifikasi â€” push browser, email, WhatsApp; SMS tersedia khusus untuk operator BPBD.
...
**Alur A â€” Warga melihat risiko & melapor kejadian**
1. Warga membuka peta publik tanpa login â†’ melihat status wilayahnya.
2. *(Opsional)* Beralih ke Mode Awam untuk tampilan simpel berbasis lokasi.
3. Warga melihat/mengalami genangan â†’ membuka form lapor.
4. Pilih lokasi â†’ isi keparahan & deskripsi â†’ unggah foto â†’ kirim.
5. Laporan berstatus "menunggu verifikasi".

**Alur B â€” Operator BPBD memvalidasi laporan**
...
**Alur C â€” BPBD Provinsi memantau & memprioritaskan**
...
**Alur D â€” Peneliti mengambil data**
...
**Alur E â€” Admin mengelola akses**
...
- **Performa**: akurasi 87%, precision 0.89, recall 0.85 â€” dievaluasi pada data historis 2018â€“2024; performa terbaik pada horizon 0â€“3 hari ke depan.
- **Sumber data**:
  - BMKG â€” data pasang surut real-time & historis
  - BIG â€” data batimetri
  - BPS â€” data kependudukan
  - Data historis kejadian banjir rob 2018â€“2024
  - Laporan ground truth warga yang telah divalidasi BPBD
- **Frekuensi update**: harian pukul 05:00 WIB; naik menjadi 2Ã—/hari saat ada peristiwa astronomis signifikan (perigee, ekuinoks).
...
- **Ketepatan Waktu**: prediksi diperbarui sesuai jadwal (lihat Bagian 8); target SLA verifikasi laporan warga 1Ã—24 jam.
...
- **Aksesibilitas Bahasa**: Mode Awam harus dapat dipahami pengguna non-teknis â€” bahasa sederhana, minim jargon spasial/statistik.
...
- Persentase laporan ground truth yang tervalidasi dalam SLA 1Ã—24 jam.
...
- Dua file pendukung desain (`_app.css`, `_nav.js`) tidak tersertakan dalam arsip mockup â€” dokumen ini disusun murni dari konten & interaksi yang terlihat di 14 layar, bukan dari spesifikasi teknis tertulis sebelumnya.
- Model ML diasumsikan dilatih di luar sistem ini; sistem mengonsumsi hasil prediksi, bukan melatih ulang model.
- Alur lupa kata sandi, reset akun, dan proses banding atas laporan yang ditolak belum tercakup di mockup â€” perlu didefinisikan lebih lanjut.
...
- Siapa yang berwenang meregenerasi API key peneliti â€” peneliti sendiri, admin, atau keduanya?
...
## 13. Lampiran â€” Pemetaan Mockup ke Modul
...
| `mockup_1_peta_publik_desktop.html` | Portal Publik | Peta interaktif â€” desktop |
| `mockup_5_peta_publik_mobile.html` | Portal Publik | Peta interaktif â€” mobile |
| `mockup_3_mode_awam_mobile.html` | Mode Awam | Status bahaya personal â€” mobile |
| `mockup_4_form_laporan_ground_truth.html` | Ground Truth | Form lapor, langkah 1â€“2 |
