...
| Kredensial email/WA/SMS | **belum ada** (mail = log, WA/SMS = mock) | â€” | Isi bagian ini saat provider asli dipasang |
...
> âš ï¸ Password pernah terpapar di percakapan 2026-07-16 â€” **rotasi ini wajib
> dilakukan** bila belum.
1. Supabase Dashboard â†’ Project Settings â†’ Database â†’ Reset database password.
2. Perbarui `.env` server (`DB_PASSWORD`) â†’ `artisan config:cache`.
3. Perbarui GitHub repo â†’ Settings â†’ Secrets and variables â†’ Actions â†’
...
5. Urutan penting: reset dulu, langsung update kedua konsumen â€” di antara
   langkah 1â€“3 aplikasi & pipeline gagal konek (downtime singkat, pilih jam
...
2. Ganti `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY` di `.env` server â†’
...
2. Tambahkan public key baru di hPanel â†’ SSH Access, hapus yang lama.
...
| Pemicu | Tindakan |
|---|---|
| Secret terpapar (chat, commit, log) | Rotasi **saat itu juga**; untuk commit di repo publik anggap bocor permanen â€” rotasi, jangan cuma hapus commit |
...
- Repo publik: secret **tidak pernah** masuk git â€” `.env` di-gitignore,
